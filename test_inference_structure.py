...
def test_inference_import():
    ...
    try:
        ...
        for name, template in templates:
            ...
            print(f"[OK] {name}: {len(template)} fields, serializable")
        ...
        print(f"[OK] START block has all required fields: {', '.join(required_start_fields)}")
        ...
        print(f"[OK] STEP block has all required fields: {', '.join(required_step_fields)}")
        ...
        print(f"[OK] END block has all required fields: {', '.join(required_end_fields)}")
        ...
        for var in env_vars:
            if hasattr(inference, var):
                ...
                print(f"[OK] {var} = {value or '(not set)'}")
            else:
        ...
        print("[OK] JSON extraction from markdown works")
        
        print("\n" + "="*70)
        print("[OK] ALL INFERENCE SCRIPT VALIDATION CHECKS PASSED")
        print("="*70)
        return 0
